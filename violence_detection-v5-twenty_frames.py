"""
violence_detection_v4-twenty_frames.py

Final Version

- Creates predictions after analyzing 20 sequential frames
"""
...
def output_prediction(img, prediction):
    ...
    cv2.putText(img, "{}".format(message), 
        (400, 30), 
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (255, 255, 255),
...
def get_boundingbox_coords(person_keypoints):
    # sort keypoints by y coord
    by_y = np.asarray(sorted(person_keypoints, key=lambda x: x[1]))
    by_y = np.asarray([keypoint for keypoint in by_y if keypoint[0] > 0 and keypoint[1] > 0]) # keypoint is zero if openpose cannot find that body part
    # sort keypoints by x coord
    by_x = np.asarray(sorted(person_keypoints, key=lambda x: x[0]))
    by_x = np.asarray([keypoint for keypoint in by_x if keypoint[0] > 0 and keypoint[1] > 0]) # keypoint is zero if openpose cannot find that body part

    # index 0 will by smallest value, index -1 will be max

    return (int(by_x[0][0]), int(by_y[0][1])), (int(by_x[-1][0]), int(by_y[-1][1]))

# show predicted aggressor
def output_aggressor(img, aggressor, curr_keypoints):
    start_point, end_point = get_boundingbox_coords(curr_keypoints[aggressor])

    if aggressor != -1:
        cv2.putText(img, "aggressor is person {}".format(aggressor), 
            (400, 50), 
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (255, 255, 255),
...
# use contact pairs for determining most likely aggressor
def predict_aggressor(recent_contact_pairs):
    # recent contact pairs contains the contact pairs from the past 20 frames
    frequency_dict = {}
    for pairs in recent_contact_pairs:
        for pair in pairs:
            p_aggressor = pair[0]
            if p_aggressor in frequency_dict:
                frequency_dict[p_aggressor] += 1
            else:
                frequency_dict[p_aggressor] = 1
...
violence_detector = joblib.load('violence_detector_seq3.pkl')
...
try:
    ...
    parser.add_argument("--image_dir", default="../../../research_dataset/social_behaviour_labeled/1_act_05-7", help="Process a directory of images. Read all standard formats (jpg, png, bmp, etc.).")
    ...
    # this list will store the data from the 20 most recent frames
    curr_batch = []

    # this list will store the contact pairs from the 20 most recent frames
    recent_contact_pairs = []
    ...
    if mode == '0':
        ...
        while rval:
            ...
            recent_contact_pairs.append(contact_pairs)
            
            # combine results before appending to curr frame
            results = process_results(angles, velocities, contact_pairs)
            
            # add frame results to curr batch
            curr_batch.append(results)

            if len(curr_batch) == 20:
                # we have 20 frames we can process, seq_results is fed to our model
                seq_results = [item for sublist in curr_batch for item in sublist]

                # feed results to model
                prediction = violence_detector.predict([seq_results])

                # erase oldest value
                del curr_batch[0]
                del recent_contact_pairs[0]
            ...
            if not args[0].no_display:
                ...
                img = output_prediction(img, prediction)

                if prediction == 1:
                    img = output_aggressor(img, predict_aggressor(recent_contact_pairs), curr_datum.poseKeypoints)
